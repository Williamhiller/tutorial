...
angular.module('global.service',['home.service','credit.service','user.service','me.service'])
    ...
    // 共通Service
    .factory('commonService',['$http','$q','$interval','$timeout','$cookieStore','$ionicPopup','validateService',function($http,$q,$interval,$timeout,$cookieStore,$ionicPopup,validateService) {
        return {
            // 检查是否登录
            isLogin: function () {
                // 声明延后监控
                if($cookieStore.get('userId') != null){
                    ...
                    $http.post("isEcbmLogin.action?time="+Math.random()).success(function(res){
            ...
            userLogout: function () {
                ...
                $http.post("userLogout.action",null,null).success(function(data){
            ...
            smsInit: function (scope, smsUrl) {
                ...
                scope.smsSend = function(voiceSms, mobileNo){
                    ...
                    if (scope.sms.isDisabled) {
                        ...
                        $http.post(scope.sms.smsUrl, $.param({"mobile_no": mobileNo, "voice_sms": scope.sms.voiceSms})).success(function(result){
                            //console.log(result)
    ...
    .factory('commonRemoteService', ['$http', '$q', function($http, $q) {
        ...
        this.getAuthAccountStatus = function(){
            //声明延后监控
            var deffered  = $q.defer();
            $http.post("accountAction!authAccountStatus.action").success(function(data){
                deffered.resolve(data);
            }).error(function(data){
                deffered.reject(data);
            })
